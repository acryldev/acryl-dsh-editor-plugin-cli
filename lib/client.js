...
window.__ModuleLoader__.load({
  // Must equal the npm package name - the client module loader keys the
  // registration by it (the 0.1.x `dsh-editor` -> `acryl-dsh-editor-plugin`
  // rename missed this, so the browser bundle "loaded without registering").
...
function loadMonaco() {
  ...
  monacoPromise = new Promise(function (resolve, reject) {
    if (typeof document === 'undefined' || typeof window === 'undefined') {
      reject(new Error('document/window is not accessible in this client; Monaco cannot load'))
      return
    }
    ...
    script.onload = function () {
      try {
        ...
        }, function (err) { reject(new Error('Monaco AMD load failed: ' + String(err))) })
      } catch (e) { reject(e) }
    }
    script.onerror = function () {
      reject(new Error('Monaco loader script failed to load (network or CSP): ' + script.src))
    }
...
function loadScriptGlobal(url) {
  return new Promise(function (resolve, reject) {
    ...
    s.onerror = function () { restore(); reject(new Error('Script failed to load: ' + url)) }
...
function apply(ctx) {
  ...
  const rpc = async function (endpoint, payload) {
    const result = await connection.rpc.call('/editor', endpoint, payload)
    if (result && result.ok === false) {
      const msg = (result.error && result.error.message) ? result.error.message : 'RPC call failed'
      throw new Error(msg)
    }
  ...
  ctx.effect(() => {
    ...
    tag.setAttribute('data-dsh-editor', '')
    tag.textContent = CSS
  ...
  // ---- Ctrl+click conversation path jump system ----
  ...
  function activateFilesTab() {
    try {
      ...
      for (const t of tabs) {
        if (t.textContent && t.textContent.trim() === 'Files') { t.click(); return }
      }
    } catch (e) {}
  }
  ...
  async function requestOpenPath(path) {
    ...
    try {
      ...
      showToast('Cannot verify path: ' + path)
      return
    }
    if (res && res.exists && res.type === 'file') {
      ...
      showToast('This is a directory and cannot be opened directly:\n' + path + '\n\nExpand it in the file tree on the left of the Files tab.')
    } else {
    ...
    try {
      ...
      for (const c of codes) {
        ...
        if (looksLikePath(text)) {
          ...
          c.setAttribute('title', 'Ctrl+click to open this file')
  ...
  function EditorPanel() {
    ...
    function loadGitStatus(rootOverride) {
      rpc('git.status', { root: rootOverride || viewState.root || '/' }).then(function (res) {
    ...
    function openDiff(path) {
      ...
      rpc('git.diff', { root: viewState.root || '/', path: path }).then(function (res) {
        if (res && !res.error) setDiffText(res.diff || '(no diff — the file may be untracked)')
        else setDiffText('(could not get diff: ' + ((res && res.error) || '') + ')')
      }).catch(function (e) { setDiffText('(could not get diff: ' + String((e && e.message) ? e.message : e) + ')') })
    }
    ...
    function gitFlag(itemPath) {
      ...
      return React.createElement('span', {
        ...
        title: 'Git status: ' + label + ' — click to view diff',
    ...
    function currentWorkspaceRoot() {
      try {
        ...
        const summary = current && snap.byId && snap.byId[current]
        return (summary && summary.cwd) ? String(summary.cwd) : null
      } catch (e) { return null }
    }

    async function initRoot() {
      try {
        const reqRoot = currentWorkspaceRoot()
        const res = await rpc('fs.init', reqRoot ? { root: reqRoot } : {})
    ...
    function mdModeLabel() {
      if (viewState.mdViewMode === 'source') return '👁 Preview'
      if (viewState.mdViewMode === 'preview') return '◫ Split'
      return '✎ Source'
    }
    ...
    function closeTab(path) {
      ...
      if (viewState.activePath === path) {
        ...
        if (next) activateFile(next.path)
          ...
          setDirty(false)
          suppressChangeRef.current = true
          if (editorRef.current) editorRef.current.setValue('// Pick a file from the file tree on the left to start editing\n')
          suppressChangeRef.current = false
        }
      }
    }
    ...
    async function doReplaceAll() {
      ...
      if (viewState.searchTruncated) {
        try { if (!window.confirm('Results were truncated; only ' + list.length + ' matches found will be replaced. Continue?')) return } catch (e) {}
      } else {
        try { if (!window.confirm('Replace ' + list.length + ' matches with the current content?')) return } catch (e) {}
      }
      ...
      try {
        ...
        setError('')
        setStatus('ready')
        if (failed) setError('Replaced ' + replaced + ' occurrences (' + failed + ' files failed)')
        loadGitStatus()
        startSearch()
      } catch (e) { setStatus('error'); setError(String((e && e.message) ? e.message : e)) }
      finally { setBusy(false) }
    ...
    React.useEffect(function () {
      ...
      loadMonaco().then(function (monaco) {
        ...
        if (!el) { setStatus('error'); setError('Editor container not ready'); return }
        const active = findFile(viewState.activePath)
        const s = editorSettings
        const createOpts = {
          value: active ? active.content : '// Pick a file from the file tree on the left to start editing\n',
    ...
    React.useEffect(function () {
      if (!sessions || !sessions.list || !sessions.list.subscribe) return
      let lastRoot = currentWorkspaceRoot()
    ...
    if (rows.length === 0) {
      resultsEl = React.createElement('div', { className: 'dsh-editor-results' },
        React.createElement('div', { className: 'dsh-editor-empty' }, searchQuery && searchQuery.trim() ? (searching ? 'Searching…' : 'No results') : 'Type a keyword to start searching')
      )
    } else {
    ...
    function metaText() {
      ...
      const scopeLabel = viewState.searchScope === 'root' ? 'Workspace' : (viewState.cwd ? 'Current project' : 'Workspace (no file open)')
      if (searchResults.length === 0 && !searchTruncated && !searching) return null
      const parts = []
      if (searching) parts.push('Searching…')
      parts.push(searchResults.length + ' results')
      parts.push(searchFiles + ' files')
      parts.push(scopeLabel + ': ' + relPath(base))
      if (searchTruncated) parts.push('Truncated; narrow the scope')
      return parts.join(' · ')
    }
    ...
    const settingsPanel = showSettings
      ? React.createElement('div', { className: 'dsh-editor-settings' },
          ...
          React.createElement('div', { className: 'dsh-editor-setting-row' },
            React.createElement('label', null, 'Tab width'),
          ...
          React.createElement('div', { className: 'dsh-editor-setting-row' },
            React.createElement('label', null, 'Word wrap'),
            React.createElement('button', { className: 'dsh-editor-setting-toggle' + (editorSettings.wordWrap ? ' on' : ''), onClick: function () { updateSetting('wordWrap', !editorSettings.wordWrap) } }, editorSettings.wordWrap ? 'On' : 'Off')
          ),
          React.createElement('div', { className: 'dsh-editor-setting-row' },
            React.createElement('label', null, 'Minimap'),
            React.createElement('button', { className: 'dsh-editor-setting-toggle' + (editorSettings.minimap ? ' on' : ''), onClick: function () { updateSetting('minimap', !editorSettings.minimap) } }, editorSettings.minimap ? 'On' : 'Off')
          )
        )
      : null
    ...
    const searchPane = React.createElement('div', { className: 'dsh-editor-search' },
      ...
      React.createElement('div', { className: 'dsh-editor-searchbar' },
        React.createElement('input', {
          ...
          placeholder: useRegex ? 'Regex (search as you type)' : 'Search text (search as you type)',
          value: searchQuery,
        ...
        React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: function () { setReplaceMode(!replaceMode) }, title: 'Toggle replace' }, replaceMode ? '🔍' : '🔄')
      ),
      replaceMode
        ? React.createElement('div', { className: 'dsh-editor-replace-row' },
            ...
            React.createElement('button', { className: 'dsh-editor-btn', onClick: doReplaceAll, disabled: !searchResults.length }, 'Replace all')
          )
        : null,
      React.createElement('div', { className: 'dsh-editor-toggles' },
        toggleBtn(caseSensitive, 'Aa', 'Match case', function () { setCaseSensitive(!caseSensitive); invalidateSearch(viewState.searchQuery) }),
        toggleBtn(wholeWord, 'ab', 'Whole word', function () { setWholeWord(!wholeWord); invalidateSearch(viewState.searchQuery) }),
        toggleBtn(useRegex, '.*', 'Use regex', function () { setUseRegex(!useRegex); invalidateSearch(viewState.searchQuery) })
      ),
      React.createElement('div', { className: 'dsh-editor-filter' },
        React.createElement('label', { className: 'dsh-editor-filter-label' }, 'Include'),
        React.createElement('input', {
          className: 'dsh-editor-search-input',
          placeholder: '*.ts, src/**',
          value: include,
      ...
      React.createElement('div', { className: 'dsh-editor-filter' },
        React.createElement('label', { className: 'dsh-editor-filter-label' }, 'Exclude'),
        React.createElement('input', {
          className: 'dsh-editor-search-input',
      ...
      React.createElement('div', { className: 'dsh-editor-scope' },
        React.createElement('button', { className: 'dsh-editor-scope-btn' + (searchScope === 'cwd' ? ' active' : ''), onClick: function () { setSearchScope('cwd'); invalidateSearch(viewState.searchQuery) } }, 'Current project'),
        React.createElement('button', { className: 'dsh-editor-scope-btn' + (searchScope === 'root' ? ' active' : ''), onClick: function () { setSearchScope('root'); invalidateSearch(viewState.searchQuery) } }, 'Workspace')
      ),
      resultsEl,
      metaText()
      ...
      ? React.createElement('div', { className: 'dsh-editor-quick-mask', onClick: function () { setQuickOpen(false) } },
          React.createElement('div', { className: 'dsh-editor-quick', onClick: function (e) { e.stopPropagation() } },
            React.createElement('input', {
              ref: quickInputRef,
              className: 'dsh-editor-search-input',
              placeholder: 'Quick open file… (current directory and subdirectories)',
              value: quickQuery,
      ...
      ? React.createElement('div', { className: 'dsh-editor-quick-mask', onClick: function () { setDiffOpen(false) } },
          React.createElement('div', { className: 'dsh-editor-diff-panel', onClick: function (e) { e.stopPropagation() } },
            React.createElement('div', { className: 'dsh-editor-diff-head' },
              ...
              React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: function () { setDiffOpen(false) } }, 'Close')
            ),
    ...
    function renderDiffLines(text) {
      if (!text) return React.createElement('div', { className: 'dsh-editor-empty' }, 'Loading…')
    ...
    const statusEl = status === 'error'
      ? React.createElement('div', { className: 'dsh-editor-status error' }, 'Error: ', String(error))
      : React.createElement('div', { className: 'dsh-editor-status ready' },
          status === 'loading' ? 'Loading Monaco…' : (busy ? 'Working…' : (activePath ? (dirty ? 'Unsaved changes' : 'Ready') : 'Ready — open a file on the left')))
    ...
    const mdArea = React.createElement('div', { className: 'dsh-editor-md-area' },
      ...
      isSplit ? React.createElement('div', { className: 'dsh-editor-md-resizer', onMouseDown: startMdSplitResize, title: 'Drag to adjust split width' }) : null,
      (isPreview || isSplit)
    ...
    return React.createElement('div', { className: 'dsh-editor-panel' },
      React.createElement('div', { className: 'dsh-editor-header' },
        React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: toggleSidebar, title: sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar' }, sidebarCollapsed ? '▶' : '◀'),
        ...
        React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: refresh, title: 'Refresh file tree' }, '🔄'),
        currentFileIsMd ? React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: cycleMdMode, title: 'Toggle Markdown view' }, mdModeLabel()) : null,
        React.createElement('button', { className: 'dsh-editor-btn ghost', onClick: function () { setShowSettings(!showSettings) }, title: 'Editor settings' }, '⚙'),
        React.createElement('button', { className: 'dsh-editor-btn', onClick: save, disabled: !activePath || !dirty, title: 'Save (' + (editorSettings.saveKey || 'Ctrl+S') + ')' }, dirty ? 'Save ●' : 'Save'),
        settingsPanel
      ),
      React.createElement('div', { className: 'dsh-editor-body' },
        sidebarCollapsed ? null : React.createElement('div', { className: 'dsh-editor-side', style: { width: sidebarWidth + 'px' } },
          React.createElement('div', { className: 'dsh-editor-lefttabs' },
            React.createElement('button', { className: 'dsh-editor-lefttab' + (!searchMode ? ' active' : ''), onClick: function () { setSearchMode(false) } }, '📁 Files'),
            React.createElement('button', { className: 'dsh-editor-lefttab' + (searchMode ? ' active' : ''), onClick: function () { setSearchMode(true) } }, '🔍 Search')
          ),
          searchMode ? searchPane : filesPane
        ),
        sidebarCollapsed ? null : React.createElement('div', { className: 'dsh-editor-resizer', onMouseDown: startResize, title: 'Drag to resize' }),
  ...
  function SettingsSection() {
    ...
    return React.createElement('div', { className: 'dsh-editor-settings-page' },
      React.createElement('div', { className: 'dsh-editor-settings-page-title' }, 'Editor'),
      row('Font size', React.createElement(NumberField, { value: s.fontSize, min: 10, max: 24, onCommit: function (v) { upd('fontSize', v) } })),
      row('Tab width', React.createElement('select', {
      ...
      toggle('Word wrap', s.wordWrap, 'wordWrap'),
      toggle('Minimap', s.minimap, 'minimap'),
      toggle('Show line numbers', s.lineNumbers, 'lineNumbers'),
      row('Font', React.createElement('input', {
        type: 'text',
        placeholder: 'Default (follow theme)',
        value: s.fontFamily,
        onChange: function (e) { upd('fontFamily', e.target.value) }
      })),
      React.createElement('div', { className: 'dsh-editor-setting-hint' }, 'Keybinding format examples: Ctrl+P, Cmd+Shift+F, F5. Case-insensitive.'),
      text('Quick open keybinding', s.quickOpenKey, 'quickOpenKey', 'Ctrl+P'),
      text('Save keybinding', s.saveKey, 'saveKey', 'Ctrl+S'),
      text('Search keybinding', s.searchKey, 'searchKey', 'Ctrl+Shift+F'),
      React.createElement('div', { className: 'dsh-editor-setting-hint' }, 'Changes apply immediately; Quick open lists files in the directory of the current file and its subdirectories.')
    )
  }

  const EDITOR_SECTION_LABEL = 'Editor'
  ...
  ctx.effect(() => registerSettingsNavIcon(), 'dsh-editor: settings nav icon')

  ctx.effect(() => installPathLinkSystem(), 'dsh-editor: path link system')

  slots.inject('conversation.view', function () {
    return slots.register(
      { name: 'conversation.view', id: 'files', order: 20, label: 'Files' },
...
const inject = ['slots', 'connection', 'timer', 'sessions']

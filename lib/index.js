...
// `connection.rpc.handle` registers its channel route on the caller's context
// (`@deepseek-ai/dsh-client-connection` >= 0.1.5 does `owner.webServer.register`
// where owner is this fiber), so `webServer` must be injected here or Cordis
// throws `cannot get property "webServer" without inject` at load.
...
export function apply(ctx) {
  ...
  const home = homedir()
  // Pin the file tree root to the user's home directory (the most stable, reliable starting point).
  // Do not depend on sandboxPolicy.workspaceRoot: it defaults to the DSH process's cwd,
  // which degenerates the tree to a handful of tool files when the process is started
  // from system directories such as node's bin.
  const root = home
  ...
  async function gitDiff(args) {
    ...
    const gitPath = await resolveGit()
    if (!gitPath) return { error: 'git unavailable' }
    ...
    try {
      ...
      if (outcome && outcome.exitCode !== 0) {
        ...
        return { diff: text || '', error: stderrText.trim() || 'git diff failed' }
      }
      return { diff: text }
  ...
  function searchPoll(args) {
    ...
    if (job.exited && !job.stdoutFlushed) {
      ...
      if (job.exitCode === 2 && !job.error) {
        ...
        job.error = stderrText.trim() || 'ripgrep failed (the regex may be invalid)'
      }
    }
  ...
  const handlers = {
    'fs.init': async (args) => {
      const reqRoot = args && args.root ? String(args.root) : ''
      return { root: reqRoot || root }
    },

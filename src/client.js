...
// src/host.js as `code.host`. See install/INSTALL.md.
...
return {
  ...
  apply(ctx) {
    ...
    async function requestOpenPath(path) {
      ...
      let res = null
      try {
        res = await host.call('fs:stat-path', { path: path })
      } catch (e) {
    ...
    function EditorPanel() {
      ...
      function loadTreeDir(path) {
        host.call('fs:list', { path: path }).then(function (res) {
          if (res && !res.error) {
      ...
      function loadGitStatus() {
        host.call('git:status', { root: viewState.root || '/' }).then(function (res) {
      ...
      function openDiff(path) {
        ...
        setDiffPath(path)
        setDiffText('')
        host.call('git:diff', { root: viewState.root || '/', path: path }).then(function (res) {
      ...
      async function initRoot() {
        try {
          const res = await host.call('fs:init', {})
      ...
      async function openFile(item) {
        ...
        try {
          ...
          try {
            const pr = await host.call('fs:project-root', { path: path })
      ...
      async function save() {
        ...
        setBusy(true)
        try {
          const res = await host.call('fs:write', { path: p, content: content })
          if (res && res.error) { setStatus('error'); setError(res.error); return }
      ...
      async function doReplaceAll() {
        ...
        setBusy(true)
        try {
          const res = await host.call('fs:search-replace', { edits: edits })
      ...
      function stopCurrentSearch() {
        ...
        if (jobRef.current) { host.call('fs:search-cancel', { jobId: jobRef.current }).catch(function () {}); jobRef.current = null }
      }
      ...
      async function startSearch() {
        ...
        let jobId = null
        try {
          const start = await host.call('fs:search-start', {
            query: q,
            root: base,
          ...
          if (mySeq !== searchSeqRef.current) {
            if (start && start.jobId) host.call('fs:search-cancel', { jobId: start.jobId }).catch(function () {})
            return
          }
        ...
        const dispose = timer.interval(function () {
          ...
          pollBusy = true
          host.call('fs:search-poll', { jobId: jobId }).then(function (res) {
            pollBusy = false
            if (mySeq !== searchSeqRef.current) return
      ...
      function openQuick() {
        ...
        if (needLoad) {
          host.call('fs:files', { root: base }).then(function (res) {
    ...
    ctx.effect(() => registerSettingsNavIcon(), 'editor: settings nav icon')

    ctx.effect(() => installPathLinkSystem(), 'editor: path link system')

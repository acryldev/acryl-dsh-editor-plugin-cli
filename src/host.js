...
// src/client.js as `code.client`. See install/INSTALL.md.
//
// It exposes private client→host JSON methods via harness.handle(...):
//   fs:*   — file read/write/list/project-root/stat/search/replace
//   git:*  — git status / diff
return {
  apply(ctx) {
    const fs = ctx.get('fs')
    ...
    harness.handle('fs:init', async () => ({ root }))

    harness.handle('fs:stat-path', async (args) => {
    ...
    harness.handle('fs:project-root', async (args) => {
    ...
    harness.handle('fs:write', async (args) => {
      if (!fs) return { error: 'fs service unavailable' }
      try {
        const target = await fs.resolve(args.path)
    ...
    harness.handle('fs:files', async (args) => {
    ...
    harness.handle('fs:search-replace', async (args) => {
      if (!fs) return { error: 'fs service unavailable' }
    ...
    harness.handle('fs:search-start', async (args) => {
    ...
    harness.handle('fs:search-poll', async (args) => {
    ...
    harness.handle('fs:search-cancel', async (args) => {
